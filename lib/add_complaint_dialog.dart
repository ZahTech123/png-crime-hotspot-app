...
class _AddComplaintDialogState extends State<AddComplaintDialog> {
  ...
  String? issueType;
  final List<XFile> _selectedImages = []; // State variable for selected images
                                     ...
                                     children: [
                                       kIsWeb 
                                         ? FutureBuilder<Uint8List>(
                                             future: widget.imageService.getImageBytes(imageFile),
  ...
  Future<void> _saveComplaint() async {
    if (formKey.currentState!.validate() && issueType != null) {
      ...
      try {
        ...
        if (_selectedImages.isNotEmpty) {
          AppLogger.d('[_saveComplaint] Uploading ${_selectedImages.length} images...');
          imageUrls = await widget.imageService.uploadImages(_selectedImages, complaintId);
          AppLogger.d('[_saveComplaint] Image URLs received: $imageUrls');
        }
        ...
        AppLogger.d('[_saveComplaint] Adding complaint to provider...');
        // Add complaint to the database via provider
        await widget.complaintProvider.addComplaint(newComplaint);
        AppLogger.d('[_saveComplaint] Complaint added via provider.');
        ...
        messenger.showSnackBar( // Use captured messenger
         ...
         AppLogger.e('[_saveComplaint] Error saving complaint', e);
         // Use captured mounted state and context objects
         if (!isMounted) return;
         messenger.showSnackBar( // Use captured messenger
