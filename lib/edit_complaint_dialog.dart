...
class _EditComplaintDialogState extends State<EditComplaintDialog> {
  ...
  Widget _buildMainImagePreview(String? existingImageUrl) {
    ...
    if (hasNewImage) {
      if (kIsWeb) {
        // For web, read bytes and display using Image.memory (optimized)
        imageWidget = FutureBuilder<Uint8List>(
          future: widget.imageService.getImageBytes(_mainImageToUpload!),
            ...
            child: Column(
                ...
                children: [
                    ...
                    Padding(
                        ...
                        child: Text(newPlatformFile.name, style: const TextStyle(fontSize: 10), maxLines: 2, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center),
                    ),
                ]
            ),
         );
        ...
        if (!isExistingUrl)
          Positioned(
            ...
            child: InkWell(
              ...
              child: Container(
                decoration: BoxDecoration(color: Colors.black.withAlpha((255 * 0.6).round()), shape: BoxShape.circle),
  ...
  Future<void> _updateComplaint() async {
    ...
    try {
      ...
      if (tempMainImageToUpload != null) { // Use the temp variable captured before clearing state
        // Use optimized background processing for image data
        final fileBytes = await widget.imageService.getImageBytes(tempMainImageToUpload);
        final mimeType = await widget.imageService.getMimeType(tempMainImageToUpload);

        // Generate simplified, unique path - IGNORING ORIGINAL NAME
        final timestamp = DateTime.now().millisecondsSinceEpoch;
        const String fileExtension = '.jpg'; // Force jpg for simplicity
        final uniqueFileName = 'main_${timestamp}_$fileExtension'; // Simple name: main_123456789.jpg
          ...
          try {
              ...
              if (pathSegments.length >= 5) { // Adjust index based on actual URL structure
                  ...
                  if (bucketName == 'complaintimages' && imagePath.isNotEmpty) {
                     ...
                     // Could not extract valid path from URL for deletion
                  }
              } else {
                   // Could not parse storage path from URL
              }
          } catch (e) {
             // Error attempting to remove existing main image from storage
          }
      }
      ...
      if (attachmentFiles.isNotEmpty) {
        for (var file in attachmentFiles) {
          ...
          final sanitizedOriginalFileName = originalFileName.replaceAll(RegExp(r'[^A-Za-z0-9_\\.-]+'), '_');
          ...
          if (kIsWeb) {
            ...
            mimeType = lookupMimeType(fileName, headerBytes: fileBytes.sublist(0, fileBytes.length > 1024 ? 1024 : fileBytes.length)) ?? 'application/octet-stream';
          } else {
            // For mobile, we could use XFile to optimize this, but since these are PlatformFiles from file_picker
            // we'll keep the direct approach for now. The main image processing is already optimized.
            final fileOnDevice = File(file.path!);
            fileBytes = await fileOnDevice.readAsBytes();
            mimeType = lookupMimeType(fileName, headerBytes: fileBytes.sublist(0, 1024)) ?? 'application/octet-stream';
          }
         ...
         // No new attachments selected for upload.
      }
       ...
       if (uploadedComplaintImagePaths.isNotEmpty) {
         try {
           await supabase.storage.from('complaintimages').remove(uploadedComplaintImagePaths);
            ...
            // Error during complaintimages storage rollback
         }
       }
       if (uploadedAttachmentPaths.isNotEmpty) {
         try {
           await supabase.storage.from('attachments').remove(uploadedAttachmentPaths);
            ...
            // Error during attachments storage rollback
         }
       }
       if (!isMounted) return;
       messenger.showSnackBar(
