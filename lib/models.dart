...
class CityComplaint {
  ...
  final int confirms; // New field for confirms
  final int commentsCount; // New field for comments count
  ...
  CityComplaint({
    ...
    this.confirms = 0, // Default to 0
    this.commentsCount = 0, // Default to 0
    this.author = 'Anonymous User', // Default author
  });

  factory CityComplaint.fromJson(String id, Map<String, dynamic> data) {
    print('[CityComplaint.fromJson] Parsing complaint ID: $id, Data: $data'); // Log incoming data
    try {
      // Log raw imageUrls data before parsing
      print('[CityComplaint.fromJson ID: $id] Raw imageUrls data: ${data['imageUrls']} (Type: ${data['imageUrls']?.runtimeType})');
      
      final parsedImageUrls = data['imageUrls'] == null || data['imageUrls'] is! List
          ? null 
          : List<String>.from((data['imageUrls'] as List<dynamic>).map((item) => item.toString())); // Ensure items are strings
      
      // Log the parsed list
      print('[CityComplaint.fromJson ID: $id] Parsed imageUrls: $parsedImageUrls');
      ...
      return CityComplaint(
        ...
        imageUrls: parsedImageUrls, // Use the parsed list
       ...
       print('[CityComplaint.fromJson] Error parsing complaint ID: $id, Error: $e, Data: $data');
  ...
  static String _normalizeStatus(String input) {
    ...
    print('Warning: Unknown status "$input", defaulting to "New"');
  ...
  static String _normalizePriority(String input) {
    ...
    print('Warning: Unknown priority "$input", defaulting to "Medium"');
    return 'Medium'; // Default fallback
  }
  // --- End Normalization Helpers ---

  static DateTime _parseDateTime(dynamic value) {
    print('[_parseDateTime] Attempting to parse: $value (Type: ${value.runtimeType})'); // Log input value and type
    try {
      if (value == null) {
         print('[_parseDateTime] Value is null, returning DateTime.now()');
         return DateTime.now();
      }
      if (value is DateTime) {
        print('[_parseDateTime] Value is already DateTime: $value');
        return value;
      }
      if (value is String) {
        print('[_parseDateTime] Value is String, parsing: $value');
        final parsed = DateTime.parse(value).toLocal();
        print('[_parseDateTime] Parsed successfully: $parsed');
        return parsed;
      }
      print('Warning: Could not parse DateTime from type ${value.runtimeType}. Input: $value');
      return DateTime.now();
    } catch (e) {
      print('Error parsing DateTime: $e. Input: $value');
      return DateTime.now();
    }
  }
